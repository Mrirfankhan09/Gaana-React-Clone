...
import RecentlyPlayed from "./Recently";
...
export default function Main({collectingData}) {
 



  const [selectedLink, setSelectedLink] = useState(0); // Initialize the first link as selected

  const linksData = [
    { content: "All" },
    { content: "Trending Song" },
    { content: "New Songs" },
    { content: "Old Songs" },
    { content: "Mood & Genres" },
    { content: "Albums" },
  ...
  function handleLinkClick(index) {
    ...
    // e.stopPropogation();
    // console.log(index)
    setSelectedLink(index); // Update the selected link index
  }
  // useEffect(()=>{
        ...
        <div class="home-page">
          <div class="page-wrapper">
            ...
            {Songs.map((element,index)=>{
              // console.log(element)
              return (<SongSection arrsongs={element} row={`row${index+1}`} index={index} collectingData={collectingData}/>)
            })}
          </div>
        </div>
      </main>
